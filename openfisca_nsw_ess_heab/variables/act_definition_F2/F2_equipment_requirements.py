...
class F2_is_registered_in_GEMS(Variable):
    value_type = bool
    entity = Building
...
class F2_complies_with_GEMS_2012_LCP(Variable):
    value_type = bool
    entity = Building
    definition_period = ETERNITY
...
class F2_meets_equipment_requirements(Variable):
    ...
    def formula(buildings, period, parameters):
        registered_in_GEMS = buildings('F2_is_registered_in_GEMS', period)
        complies_with_GEMS_2012 = buildings('F2_complies_with_GEMS_2012_LCP', period)
        IPLV_higher_than_baseline = buildings('F2_IPLV_10_percent_higher_than_baseline', period)
