...
class F14_gas_savings(Variable):
    value_type = float
    entity = Building
    ...
    def formula(buildings, period, parameters):
        current_capacity = buildings('F14_current_equipment_capacity', period)
        operating_pressure = buildings('F14_operating_pressure', period)
...
class F14_current_equipment_capacity(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
...
class F14_operating_pressure(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
