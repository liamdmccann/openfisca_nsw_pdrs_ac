...
class F5_electricity_savings(Variable):
    value_type = float
    entity = Building
    ...
    def formula(buildings, period, parameters):
        ...
        nominal_input_power = select([input_power <= 34.00,
                                    ...
                                    "more_than_34_kW"])
        coefficient_a = parameters(period).HEAB.F5.nominal_input_power.coefficient_a[nominal_input_power]
        coefficient_b = parameters(period).HEAB.F5.nominal_input_power.coefficient_b[nominal_input_power]
